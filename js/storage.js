...
function loadOrSeedDb() {
  try {
    ...
    if (saved) {
      ...
      tables.forEach((t) => {
        // Only overwrite if parsed data is valid; keeps empty arrays as fallback
        if (parsed[t] !== undefined) db[t] = parsed[t];
      });

      // Restore session; fall back gracefully when IDs no longer exist
      const uid = localStorage.getItem("current_user_id");
      const cfid = localStorage.getItem("current_cash_fund_id");
      if (uid)
        currentUser = db.users.find((u) => u.id === parseInt(uid)) || null;
      if (cfid)
        currentCashFund =
          db.cash_fund.find((f) => f.id === parseInt(cfid)) || null;
      if (!currentUser)
        currentUser = db.users.find((u) => !u.is_deleted) || null;
      if (!currentCashFund) {
        currentCashFund =
          db.cash_fund.find((f) => f.closed_at === null) || null;
      }
      return true;
    }
  } catch (e) {
  ...
  // loadOrSeedDb: Falls through to seed on corrupt or missing storage.
  seedData();
  persistDb();
  return false;
}
